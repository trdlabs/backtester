...
**Interfaces:**
- Consumes: `ComputeLockStore` (Task 1), `InternalJobStatus` (Task 4), `AppConfig` flags (Task 3), and the `JobRow`/`JobRowPatch` fields + `claimNextQueued(...,{coalesceEnabled})` from **Task 5** (already present — those patch fields are applied by `InMemoryJobStore.transition`/`PgJobStore.transition`).
...
- `coalesceOn = deps.coalesceEnabled === true && deps.computeLock !== undefined && dedupOn && deps.lease !== undefined && claimed.request.bypassCache !== true` (dedupOn already computed in the gate; coalescing requires dedup on and a `deps.lease.workerId`).
- **`bypassCache` bypasses coalescing entirely** (it forces a fresh compute by contract): NO `resultCache.lookup`, NO `computeLock.acquire`, NO `waiting_for_compute` — the engine runs fresh (and a successful run may still populate `result_cache`). This mirrors how `bypassCache` already skips the completed-cache lookup; `coalesceOn` excludes it via `claimed.request.bypassCache !== true`. A `bypass` run still charges the engine attempt (see 3c — attempts charge on ALL engine paths).
...
describe('coalescing gate — momentum', () => {
  ...
  it('bypassCache=true bypasses coalescing: even with an ACTIVE lock the run does NOT defer — engine runs fresh', async () => {
    ...
    const { store, deps } = makeCtx({ dedupEnabled: true, coalesceEnabled: true, computeLock: lock });
    const runSpy = vi.spyOn(runBacktestModule, 'runBacktest');
    // Hold an alive lock for the identity — a normal run would defer; a bypassCache run must NOT.
    await seedActiveLockForMomentum(lock, deps, 'run-other', 10_000_000);
...
```
> `enqueueBypass` = `enqueue` but the queued job's `request` carries `bypassCache: true` (mirror `momentumJob` with the flag set).
...
```ts
  const coalesceOn = deps.coalesceEnabled === true && deps.computeLock !== undefined && dedupOn
    && deps.lease !== undefined && claimed.request.bypassCache !== true; // bypassCache forces fresh compute — no coalescing
...
> `store.transition` / `JobRowPatch` already accept the new fields (`computeIdentity`, `waitDeadlineMs`, `computeWaitAttempts`, `engineAttemptCharged`, `attempts`) — they were added in **Task 5** (`JobRow`/`JobRowPatch` fields + patch application in both `InMemoryJobStore.transition` and `PgJobStore.transition`). This task only consumes them.
...
```
Add `registerLeader?`/`unregisterLeader?` optional callbacks to `WorkerDeps`, wired by `runWorkerLoop` into `deps` before draining; the Task 6 gate calls `deps.registerLeader?.(identity)` on lock-win and `deps.unregisterLeader?.(identity)` in the `finally`.
...
- Deferred attempt charging in claim + `JobRow`/`JobRowPatch` fields (INV-5) → Task 5. ✅
- `computeIdentity` gate, leader/follower, engine-commit charge on all engine paths, `bypassCache` bypasses coalescing (INV-1, INV-5) → Task 6. ✅
...
- Coalescing OFF no-op (INV-6) → asserted in Tasks 6 (goldens) + 5 (claim-off attempts) + reaper-off path; `pnpm check` in 9. ✅
