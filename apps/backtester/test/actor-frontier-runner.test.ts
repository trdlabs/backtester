...
describe('ИНТЕГРАЦИЯ: запись раннера проходит существующую проекцию', () => {
  ...
  it('частичный выход СВОДИТСЯ ТОЧНО — канон лежит на решётке леджера', async () => {
    // Здесь стояла характеризующая проба на открытый дефект: гейт сравнивал через `Object.is`
    // артефактную сводку сделок с якорем леджера, а они лежат на РАЗНЫХ решётках округления
    // (float64 в `grossOnClose`, двойной выход апорционированной комиссии, округление на каждом
    // филле против округления на каждой сделке). На круглых числах фикстур совпадение было удачей;
    // на настоящих размерах заявки оно пропало — 27.98060422525056 против 27.980604225250556.
    //
    // Решение владельца: допуск не вводить, дать движковую каноническую операцию. Она появилась в
    // `@trdlabs/engine@0.11.0` (`canonicalRealizedPnl`), накапливается теми же вызовами
    // `accrueRealized`, что `applyFill`, и потому равна леджеру побитово по построению. Проба
    // перевёрнута: тот же прогон, что раньше падал, теперь проходит проекцию целиком.
    let step = 0;
    const { run } = makeRun({
      lookback: 0,
      barCount: 6,
    ...
    const record = await run();
    // Позиция закрыта НЕ ПОЛНОСТЬЮ — именно остаток разводил два порядка суммирования.
    expect(record.finalLedger.qty).toBeGreaterThan(0);
    const artifacts = projectActorRun(record);
    expect(artifacts.validationIssues).toEqual([]);
    expect(artifacts.trades).toHaveLength(1);
    // И сделка действительно частичная: проба проверяет тот самый случай, а не соседний.
    expect(artifacts.trades[0]!.closeReason).toBe('strategy_exit');
  });
});
