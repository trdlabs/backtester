...
describe('actor-путь доезжает до настоящего RunAccumulators', () => {
  ...
  it('многосимвольный прогон ИДЁТ, и каждый актор получает СВОЙ символ', async () => {
    ...
    expect(out.refusal).toBeNull();
    expect(out.records!.map((r) => r.actorId).sort()).toEqual(
      [actorIdFor('BTCUSDT'), actorIdFor('ETHUSDT')].sort(),
    );
    const acc = out.accumulators!;
    ...
    // уровням (BTC от 100, ETH от 150) ровно ради этого. Актор BTC, накормленный свечами ETH,
    // исполнился бы по 151 при том же имени и том же числе заявок.
