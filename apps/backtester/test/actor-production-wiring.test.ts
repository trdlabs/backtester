...
describe('actor-путь доезжает до настоящего RunAccumulators', () => {
  ...
  it('многосимвольный прогон ИДЁТ, и каждый актор получает СВОЙ символ', async () => {
    // Эта проба раньше пиннила сплошной отказ. Развилка, из-за которой он стоял, разрешена
    // владельцем (cc#395 §5): требование называет КОНКРЕТНЫЙ инструмент и обслуживает только его.
    //
    // Опасение снятого отказа было названо в его же комментарии: «исполнит стратегию, написанную
    // под BTC, на ETH». Здесь оно и проверяется — не тем, что прогон прошёл, а тем, ЧЬЯ заявка
    // чья. `executorPlacingOnce` метит clientOrderId символом, с которым поднят актор, поэтому
    // перепутанная привязка требования к ленте даёт две заявки от одного символа, а не по одной
    // от каждого. Число заявок при этом остаётся тем же — на него смотреть бесполезно.
    // ПОРЯДОК ЗДЕСЬ ОБРАТНО-АЛФАВИТНЫЙ, И ЭТО НЕ КАПРИЗ.
    //
    // Пока порядок объявления, порядок прогона и алфавитный совпадают, отображение «по имени»
    // неотличимо от отображения «по позиции»: тождественная перестановка выглядит правильной.
    // Целый класс дефектов — `актор[i] → лента[i]`, где одна сторона отсортирована, а другая
    // взята в порядке объявления, — на совпадающей фикстуре зеленеет. Развёрнутый порядок
    // ломает совпадение: любое позиционное отображение теперь скрещивает ленты. Мутация M6.
    ...
    expect(out.refusal).toBeNull();
    // БЕЗ `.sort()`: порядок акторов обязан следовать ЗАПРОСУ, а не алфавиту. Отсортировать обе
    // стороны значило бы согласиться с реализацией, которая молча нормализует порядок прогона.
    expect(out.records!.map((r) => r.actorId)).toEqual([
      actorIdFor('ETHUSDT'),
      actorIdFor('BTCUSDT'),
    ]);
    const acc = out.accumulators!;
    expect(acc.orders.map((o) => o.id).sort()).toEqual(['BTCUSDT-o1', 'ETHUSDT-o1']);
    // Уровни назначены по ПОЗИЦИИ в `symbols`, поэтому при обратном порядке ETH идёт от 100,
    // а BTC — от 150. Пара «имя ↔ цена» ниже и есть предмет проверки.
    // А ВОТ ЭТО — единственная проверка, которая ловит ПЕРЕКРЁСТНУЮ привязку. Имя заявки берётся
    // из символа, с которым поднят актор, и при спутанных лентах осталось бы прежним. Цена
    // исполнения берётся из ленты, которую актор реально получил, — и фикстура разводит ленты по
    // уровням ровно ради этого. Актор, накормленный чужими свечами, исполнился бы по чужой цене
    // при том же имени и том же числе заявок.
    expect(acc.fills.map((f) => `${f.orderId}@${f.fillPrice}`).sort()).toEqual([
      'BTCUSDT-o1@151',
      'ETHUSDT-o1@101',
    ]);
    // Аккумуляторы ОДНИ на прогон, а не по штуке на актор: иначе результат собрался бы по
    // подмножеству, и `orderIndex` разошёлся бы с `orders` молча.
    expect(acc.orderIndex.size).toBe(acc.orders.length);
  });
