...
describe('всё, что не поддержано, отвергается — и отказ НЕ ОТДАЁТ ничего пригодного', () => {
  ...
  // ПРЕДУСЛОВИЕ ЭТОЙ ПРОБЫ ИЗМЕНИЛОСЬ, и она переписана под новое, а не удалена и не ослаблена.
  // Прежде четыре вида отвергались как НЕПОДДЕРЖАННЫЕ; теперь они поддержаны, и отвергать их надо
  // по настоящей причине — лента этих проб их не несёт. Смысл случая сохранён дословно: объявленное
  // и недоставляемое обязано быть отказом, потому что молчаливый запуск даёт правдоподобные числа
  // на входе, которого стратегия не получила.
  it.each(['open_interest', 'liquidations', 'taker_volume', 'funding'])(
    'вид «%s» объявлен, но лента его не несёт — отказ по составу ленты',
    (kind) => {
      refuses(
        [supported({ kind, scope: 'aggregate', unit: 'usd', form: 'rate' } as Partial<MarketDataRequirement>)],
        /лента прогона его не несёт вовсе/,
      );
    },
  );
  ...
  // ПРЕДУСЛОВИЕ ИЗМЕНИЛОСЬ СТУПЕНЬЮ 1, проба переписана под новое, а не ослаблена.
  //
  // Прежде требование чужого символа отвергало прогон прямо здесь. При N акторах это означало бы,
  // что второй актор не поднимется никогда: его собственное требование выглядит «чужим» для первой
  // ленты. Теперь чужое ОТБИРАЕТСЯ, и если для этой ленты не осталось ничего — отказ наступает по
  // настоящей причине: доставлять нечего.
  //
  // Молчаливой потери при этом нет, и это проверяется отдельно на уровне прогона целиком
  // (`actor-multi-symbol.test.ts`): требование на символ ВНЕ прогона и символ БЕЗ требований
  // отвергаются там, где видны все символы сразу. Здесь их не видно по построению.
  it('манифест, не объявивший НИЧЕГО для этой ленты, отвергается: доставлять нечего', () => {
    refuses(
      [supported({ instrument: { venue: 'binance', symbol: 'ETHUSDT' } } as Partial<MarketDataRequirement>)],
      /для символа BTCUSDT не объявлено ни одного требования/,
    );
  });
