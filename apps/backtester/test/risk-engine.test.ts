...
// Ф3: the decision vocabulary the risk engine consumes is owned by `@trdlabs/engine`
// (`src/contract/index.ts`), so these characterization tests type against the core's decisions
// rather than the research contract's near-identical copy.
import type { StrategyDecision } from '@trdlabs/engine';
...
// Ф3 / SSOT decision 3: risk owns sizing outright and returns a finished `notional` instead of a
// `sizingPct`, sized off MARK-TO-MARKET equity handed in via `RiskContext`. Every expectation below
// that read `sizingPct: p` now reads `notional: p × EQUITY` — the same authority, materialized.
const EQUITY = 1000;
const flat = { equity: EQUITY, openPositions: 0 } as const;
const held = { equity: EQUITY, openPositions: 1 } as const;

describe('RiskEngine — enter', () => {
  it('accepts a within-profile entry with full-notional sizing', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate(enter(), 0, flat)).toMatchObject({
      action: 'accept',
      notional: 1000,
  ...
  it('rejects a side not in allowedSides', () => {
    expect(new RiskEngine(LONG_ONLY_RISK).evaluate(enter({ side: 'short' }), 0, flat)).toMatchObject({
      action: 'reject',
  ...
  it('rejects when openPositions has reached maxConcurrentPositions (portfolio-wide)', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate(enter(), 3, held)).toMatchObject({
      action: 'reject',
  ...
  it('clamps an out-of-bounds stop hint to the profile bound and records the clamp', () => {
    expect(new RiskEngine(TIGHT_STOP_RISK).evaluate(enter({ stop: 0.5 }), 0, flat)).toMatchObject({
      action: 'clamp',
      notional: 1000,
  ...
  it('accepts in-bounds protection hints and carries the normalized stop/take', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate(enter({ stop: 0.05, take: 0.1 }), 0, flat)).toMatchObject({
      action: 'accept',
      stop: 0.05,
      take: 0.1,
...
describe('RiskEngine — exit normalization (R3)', () => {
  it('full exit (no percent) is always accepted', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate(exit(), 1, held)).toMatchObject({
      action: 'accept',
  ...
  it('rejects a non-positive or non-finite exit percent', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate(exit(0), 1, held)).toMatchObject({ action: 'reject', record: { reason: 'invalid_exit_percent' } });
    expect(new RiskEngine(DEFAULT_RISK).evaluate(exit(Number.NaN), 1, held)).toMatchObject({ action: 'reject', record: { reason: 'invalid_exit_percent' } });
  });

  it('clamps percent ≥ 100 to a full exit', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate(exit(150), 1, held)).toMatchObject({
      action: 'clamp',
  ...
  it('accepts 0<p<100 as a partial exit with closeFraction = p/100', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate(exit(25), 1, held)).toMatchObject({
      action: 'accept',
      closeFraction: 0.25,
...
describe('RiskEngine — update_protection (US3)', () => {
  it('rejects update_protection while flat (no open position)', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate(updateProtection(0.05), 2, flat)).toMatchObject({
      action: 'reject',
  ...
  it('accepts an in-bounds protection update', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate(updateProtection(0.05), 2, held)).toMatchObject({
      action: 'accept',
      stop: 0.05,
  ...
  it('clamps an out-of-bounds protection update', () => {
    expect(new RiskEngine(TIGHT_STOP_RISK).evaluate(updateProtection(0.5), 2, held)).toMatchObject({
      action: 'clamp',
      stop: 0.02,
      record: { action: 'clamp', reason: 'hints_clamped', clamped: [{ field: 'stop', from: 0.5, to: 0.02 }] },
    });
...
describe('RiskEngine — add_to_position limits (R4)', () => {
  const pos = { size: 0, entryPrice: 0, addCount: 0 };

  it('rejects add while flat or with no position context', () => {
    // flat (openPositions 0) and «open but no position view» are both rejects, unchanged by Ф3.
    expect(new RiskEngine(DCA_RISK).evaluate(add('dca'), 0, { ...flat, position: pos })).toMatchObject({ action: 'reject', record: { reason: 'add_without_position' } });
    expect(new RiskEngine(DCA_RISK).evaluate(add('dca'), 0, held)).toMatchObject({ action: 'reject', record: { reason: 'add_without_position' } });
  });
  ...
  it('rejects when the mode is not permitted by the profile (no limits declared)', () => {
    ...
    expect(new RiskEngine(DEFAULT_RISK).evaluate(add('dca'), 0, { ...held, position: pos })).toMatchObject({ action: 'reject', record: { reason: 'dca_not_permitted' } });
    expect(new RiskEngine(DEFAULT_RISK).evaluate(add('scale_in'), 0, { ...held, position: pos })).toMatchObject({ action: 'reject', record: { reason: 'scale_in_not_permitted' } });
  });

  it('rejects once maxAdds is exhausted', () => {
    expect(new RiskEngine(TIGHT_ADD_RISK).evaluate(add('dca'), 0, { ...held, position: { ...pos, addCount: 1 } })).toMatchObject({
      action: 'reject',
  ...
  it('clamps an oversized sizingHint to maxAddNotionalPct', () => {
    ...
    expect(new RiskEngine(DCA_RISK).evaluate(add('dca', 0.5), 0, { ...held, position: pos })).toMatchObject({
      action: 'clamp',
      mode: 'dca',
      notional: 250,
  ...
  it('accepts an add within all limits', () => {
    expect(new RiskEngine(DCA_RISK).evaluate(add('dca', 0.2), 0, { ...held, position: pos })).toMatchObject({
      action: 'accept',
      mode: 'dca',
      notional: 200,
...
describe('RiskEngine — other kinds', () => {
  it('defensively accepts kinds that never reach risk (idle) as a no-op', () => {
    expect(new RiskEngine(DEFAULT_RISK).evaluate({ kind: 'idle' }, 5, flat)).toMatchObject({
      action: 'accept',
