...
export class IsolateModuleExecutor implements ModuleExecutor {
   ...
   *
   * Отсюда `disposeActor` в изоляте на пути отказа ПОСЛЕ удачного захода: если харнесс успел
   * завести слот, а ответ не доехал (таймаут, порча ответа), слот остался бы жить за границей
   * навсегда. Случай не гипотетический — это ровно та разница между «слот не заводился» и «слот
   * заведён, но неадресуем», которую снаружи не отличить.
   */
  async createActor(source: ActorSource, init: ActorInit): Promise<ActorExecutionHandle> {
    ...
    if (!r.ok) {
      // Заход не удался — слот МОГ быть заведён (таймаут после записи). Прибираем за собой.
