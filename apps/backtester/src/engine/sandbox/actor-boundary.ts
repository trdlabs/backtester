 ...
 *
 * ОТЛИЧИЕ, ВНОСИМОЕ ИМЕННО ЭТИМ МЕСТОМ: `openOrderViewOf` бросает на заявке `limit`/`stop_market`
 * без цены (инвариант хоста). На прямом пути бросок случился бы, только если автор СПРОСИЛ заявки;
 * здесь — всегда. Это ужесточение, а не расхождение семантики: состояние, вызывающее бросок,
 * незаконно в обоих случаях, и узнать о нём раньше лучше, чем позже.
 *
 * Утверждать «отличие одно» этот файл не вправе — он видит только свою половину. Полный список
 * того, чем sandbox-дорога строже прямой (заморозка `init` и снимков, ревалидация батча схемой),
 * ведётся в `docs/superpowers/specs/2026-08-16-083-s3-actor-isolate-boundary.md` §8.
 */
