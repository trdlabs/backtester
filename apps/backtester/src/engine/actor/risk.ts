...
/**
 * НАПРАВЛЕНИЕ заявки относительно текущей экспозиции — выводится из знака, а не объявляется.
 *
 * `close` означает «направлена ПРОТИВ экспозиции», а НЕ «закроет позицию». Разница существенна и
 * однажды была дефектом: размера в этой классификации нет, поэтому заявка на нотионал больше
 * позиции пересекает ноль и открывает противоположную. Значения union'а совпадают с движковым
 * `SimulatedOrder.intent` и потому не переименовываются — вместо этого правило «выход только через
 * reduceOnly» держит `evaluateActorPlace`.
 */
export type ActorOrderIntent = 'open' | 'add' | 'close';

/**
 * Направление заявки. Единственный источник для обоих потребителей — риска и записи прогона.
 *
...
export function evaluateActorPlace(
  ...
  // ── Противоположная сторона ≠ закрытие ────────────────────────────────────────
  //
  // САМЫЙ ОПАСНЫЙ СЛУЧАЙ ФАЙЛА, и выглядит он безобидно. Заявка против позиции классифицируется
  // как `close` по ЗНАКУ, а размера в этой классификации нет вовсе. Заявка на нотионал БОЛЬШЕ
  // позиции пересекает ноль: движок закрывает старую экспозицию и остатком открывает
  // ПРОТИВОПОЛОЖНУЮ (`applyFill`, случай 3 — флип). То есть под видом выхода открывается новая
  // позиция — мимо `allowedSides`, мимо счётчика позиций и мимо потолка экспозиции, потому что все
  // три проверяются только на ветке `open`.
  //
  // Проверить размер здесь и «разрешить, если не пересечёт» НЕЛЬЗЯ: нотионал переводится в базовый
  // размер ценой ИСПОЛНЕНИЯ, которой в момент подачи не существует — её не будет до матча и сдвига
  // на проскальзывание. Любая арифметика на этом месте была бы оценкой, выданной за факт.
  //
  // Поэтому в S3 закрытием считается ТОЛЬКО `reduceOnly`: у неё пересечение нуля невозможно по
  // построению — движок клампит её знаковым остатком позиции и отменяет, если сокращать нечего.
  if (intent === 'close' && command.reduceOnly !== true) {
    const reason = 'opposite_side_requires_reduce_only';
    return {
      kind: 'reject',
      reason: formatRiskRefusal(
        reason,
        `заявка ${command.side} против позиции ${state.ledger.qty} не помечена reduceOnly: её ` +
          'нотионал может превысить позицию, пересечь ноль и открыть противоположную — то есть ' +
          'открыть позицию, минуя проверки стороны, счётчика и потолка. Размер в базовой валюте ' +
          'до исполнения неизвестен, поэтому «не пересечёт» здесь недоказуемо',
      ),
  ...
  //
  // Клампить выход по потолку экспозиции было бы прямым дефектом: потолок ограничивает, сколько
  // позиции можно НАБРАТЬ, а урезание выхода заперло бы капитал в активе, из которого нет выхода.
  // Проверять сторону на выходе — то же самое другими словами: `sell` при длинной позиции это
  // выход, а не short.
  //
  // Выход к этому месту уже сужен до `reduceOnly` (см. выше), поэтому «открытие под видом выхода»
  // здесь невыразимо: у reduceOnly-заявки пересечение нуля невозможно по построению.
  if (intent === 'open') {
    ...
    if (profile.maxConcurrentPositions !== undefined) {
      // То же правило, что и для потолка: сравнение с `NaN` ложно ВСЕГДА, поэтому негодное
      // значение не «ослабило бы» лимит, а выключило бы его целиком и незаметно.
      if (!Number.isInteger(profile.maxConcurrentPositions) || profile.maxConcurrentPositions < 0) {
        throw new Error(
          `evaluateActorPlace: профиль ${profile.id}@${profile.version} миновал гейт совместимости ` +
            `с maxConcurrentPositions=${profile.maxConcurrentPositions}`,
        );
      }
