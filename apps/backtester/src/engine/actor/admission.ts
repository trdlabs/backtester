...
/**
 * Требование объявлено СВЯЗАННОЙ ветвью привязки: венью названо, символ приносит прогон.
 *
 * Различитель — присутствие `symbolFrom`, а не отсутствие символа. Форма «символа нет ⇒ значит
 * связанное» отвергнута в контракте осознанно: отсутствие ключа как разрешающее значение
 * неотличимо от ПОТЕРИ поля. Здесь то же самое читается с другой стороны — потерянный
 * `symbolFrom` превращает требование в фиксированное, а фиксированное без символа контракт не
 * пропускает вовсе.
 */
function isBoundRequirement(req: MarketDataRequirement): boolean {
  return (req as { readonly symbolFrom?: unknown }).symbolFrom === 'actor';
}

/**
 * Символ, который требование обслуживает НА ЭТОЙ ленте, либо `undefined`, если не на этой.
 *
 * Единственное место, где две ветви привязки расходятся, и потому единственное, где их можно
 * спутать. Всё остальное — венью, интервал, ревизии, прогрев — у ветвей общее и проверяется
 * одинаково; заводить второй проход по требованиям ради второй ветви значило бы завести вторую
 * реализацию тех же проверок, которая разойдётся с первой на первой же правке.
 *
 * СВЯЗАННАЯ ВЕТВЬ ПРИ ОДНОМ АКТОРЕ ТОЖДЕСТВЕННА ФИКСИРОВАННОЙ на его символе — это требование §6
 * плана, и здесь оно выполняется по построению, а не совпадением: обе ветви возвращают ровно
 * `tape.symbol`, просто одна сначала сверяет его с объявленным.
 */
export function requirementSymbolFor(
  req: MarketDataRequirement,
  tapeSymbol: string,
): string | undefined {
  if (isBoundRequirement(req)) return tapeSymbol;
  return req.instrument.symbol === tapeSymbol ? tapeSymbol : undefined;
}
...
export function admitActorMarketData(
  ...
  for (const req of requirements) {
    ...
    // ДВЕ ВЕТВИ ПРИВЯЗКИ (§5 плана, контракт 017.6) — и обе разрешаются ЗДЕСЬ, одним выражением.
    //
    // Допуск вызывается НА КАЖДЫЙ символ прогона и видит ровно одну ленту:
    //
    //   • ФИКСИРОВАННАЯ ветвь называет конкретный символ и обслуживает только его. Требование
    //     чужого символа ОТБИРАЕТСЯ, а не отвергает прогон: при N символах требования манифеста
    //     относятся к разным акторам, и отказ здесь означал бы, что второй актор не поднимется
    //     никогда — его собственное требование выглядит «чужим» для первой ленты.
    //   • СВЯЗАННАЯ ветвь символа не называет вовсе и обслуживает ТОТ, ПО КОТОРОМУ ИДЁТ ЛЕНТА.
    //     Пропускать её нельзя ни при каком символе, иначе актор объявил вход и не получил его.
    //
    // Молчаливой потери здесь нет: требование на символ ВНЕ прогона и символ БЕЗ требований
    // отвергаются раньше, на уровне прогона целиком, где видны оба.
    const servedSymbol = requirementSymbolFor(req, tape.symbol);
    if (servedSymbol === undefined) continue;
    ...
    if (req.kind === SUPPORTED_KIND) {
      ...
      requirement = Object.freeze({
        ...
        // ОБСЛУЖИВАЕМЫЙ символ, а не объявленный: у связанной ветви объявленного нет вовсе.
        symbol: servedSymbol,
      ...
      requirement = Object.freeze({
        ...
        kind: req.kind,
        venue: req.instrument.venue,
        // ОБСЛУЖИВАЕМЫЙ символ, а не объявленный (см. `requirementSymbolFor`).
        symbol: servedSymbol,
        intervalUs: Number(req.interval),
        lookback: req.lookback,
        scope: 'aggregate',
  ...
  // НИ ОДНО ТРЕБОВАНИЕ НЕ ОТНОСИТСЯ К ЭТОЙ ЛЕНТЕ. Прогон целиком отвергает такой случай раньше
  // (символ без требований виден только там, где известны все символы), но допуск обязан быть
  // САМОСТОЯТЕЛЬНЫМ: он не вправе полагаться на то, что кто-то раньше уже проверил.
  //
  // Без этой ветви `Math.max()` от пустого списка дал бы `-Infinity` порогом прогрева — то есть
  // «готов с самого начала» у актора, которому нечего доставлять. Отказ выглядел бы успехом.
  if (bindings.length === 0) {
    return no(
      `${label(strategy)}: для символа ${tape.symbol} не объявлено ни одного требования — ` +
        'актор поднялся бы и не получил ни одного рыночного события',
    );
  }

  // Максимум, а не сумма и не первый: готовность у актора одна, и наступает она, когда набрана
  // история КАЖДОГО требования.
