...
export function admitActorMarketData(
  ...
  for (const req of requirements) {
    ...
    // ТРЕБОВАНИЕ ЧУЖОГО СИМВОЛА ОТБИРАЕТСЯ, А НЕ ОТВЕРГАЕТ ПРОГОН — ступень 1 multi-symbol.
    //
    // Допуск вызывается НА КАЖДЫЙ символ прогона и видит ровно одну ленту. При N символах
    // требования манифеста относятся к разным акторам, и отказ здесь означал бы, что второй актор
    // не поднимется никогда: его собственное требование выглядит «чужим» для первой ленты.
    //
    // Это и есть ратифицированное чтение фиксированной ветви (§5 плана): требование называет
    // КОНКРЕТНЫЙ инструмент и обслуживает только его. Молчаливой потери здесь нет — требование на
    // символ ВНЕ прогона и символ БЕЗ требований отвергаются раньше, на уровне прогона целиком,
    // где оба видны; сюда доходит только законное распределение по акторам.
    if (req.instrument.symbol !== tape.symbol) continue;
