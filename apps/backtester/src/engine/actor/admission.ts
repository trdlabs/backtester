...
export function proveCandleVenue(dataset: {
  ...
  if (dataset.candleVenueHomogeneous === false) {
    return {
      ...
      reason:
        `датасет «${dataset.datasetRef}»: источник свечей внутри суток НЕОДНОРОДЕН — файлы дня ` +
        'назвали разные венью, а поминутного разбиения канон не несёт. Это знание о данных, а не ' +
        'его отсутствие: чинится пересборкой суток из однородного источника, а не дозаписью ' +
        'метаданных',
    };
  }
