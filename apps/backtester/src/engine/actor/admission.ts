...
export function admitActorExecutor(
  ...
  // ПРИЧИНА ОБНОВЛЕНА ВМЕСТЕ С КОДОМ, и это не косметика. Прежняя редакция говорила «проекция
  // ledger → артефакты не подключена»; с подключением проекции и внутреннего раннера это стало
  // НЕПРАВДОЙ. Отказ, называющий несуществующую причину, отправляет оператора чинить то, что уже
  // работает, и прячет то, что действительно не сделано, — он вреднее молчания, потому что выглядит
  // информативным.
  //
  // Что не сделано на самом деле: продовая точка вызова. `runEventDrivenSymbol` и агрегация готовы
  // и покрыты гейтами, но `runner.ts` ещё не собирает из их артефактов `BacktestRunResult`, и
  // канал происхождения свечей от датасета к раннеру не проложен. До обоих условий actor-путь
  // остаётся fail-closed: успешный прогон, чей результат никуда не доезжает, хуже отказа.
  return refuse(
    `lifecycle: 'event_driven' (${id}) совместим со всеми условиями хоста, а проекция и внутренний ` +
      'раннер подключены — но продовая точка вызова ещё не собирает из их артефактов результат ' +
      'прогона, и происхождение свечей не доезжает от датасета к допуску. Успешный прогон, чей ' +
      'результат никуда не доезжает, хуже отказа: у отказа есть причина и адресат.',
  );
}
