...
//     `canonicalRealizedPnl` — величину, лежащую на решётке округления леджера и потому равную ему
//     ПОБИТОВО по построению, но собранную из модели эр деривации (цена входа эры, закрываемый
//     размер, границы эр). Ошибка в модели эр видна ровно на этом равенстве.
//
// ПОЧЕМУ НЕ `reconcileRealizedPnl` И ПОЧЕМУ НЕ ДОПУСК. Та сводит артефактные величины сделок по
// legacy-соглашению о комиссии и сходится с леджером МАТЕМАТИЧЕСКИ, но не побитово: решётки
// округления разные (float64 в `grossOnClose`, двойной выход апорционированной комиссии,
// округление на каждом филле против округления на каждой сделке). На круглых числах разница не
// видна, на настоящих — 3.55e-15, и `Object.is` отвергал корректный прогон. Допуск запрещён
// решением владельца: он прячет ровно тот класс дефекта, ради которого сверка существует.
// Каноническая операция появилась в `@trdlabs/engine@0.11.0` (engine#25).
//
 ...
 *
 * Сравнение ТОЧНОЕ и остаётся точным: подаётся `canonicalRealizedPnl`, лежащая на решётке
 * округления леджера. Допуск сюда не вводится — он пропустил бы ошибку апорционирования, которая
 * живёт как раз в последних разрядах.
 *
...
export function projectActorRun(record: ActorExecutionRecord): ActorRunArtifacts {
  ...
  assertTradesReconcile(canonicalRealizedPnl(derivation), record.finalLedger.realizedPnl);
