...
export interface ActorFrontierRecord {
   ...
   *
   * Проекцией не читается — проверяется: непрерывность actor-local `seq` (§3.5) означает, что
   * значение не может убывать между frontier'ами. Разрыв здесь — признак того, что раннер потерял
   * или переиграл события, и обнаружить это дешевле на проекции, чем на расхождении чисел.
   */
