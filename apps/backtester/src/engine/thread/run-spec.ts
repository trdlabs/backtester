...
export interface ThreadRunFlags {
   ...
   *
   * Раньше поле здесь отсутствовало намеренно — `StrategyRunDeps` его не принимал, и держать
   * неработающий knob было хуже, чем не держать. Теперь принимает, и отсутствие поля означало бы
   * обратное: поток молча морозил бы там, где главный поток уже не морозит, и два прод-пути
   * разошлись бы по стоимости при одинаковом результате.
   */
  readonly contextFreeze?: boolean;
  /**
   * 083 S3: разрешение раскатки `lifecycle: 'event_driven'`. Absent ⇒ `false`.
   *
   * Обязано быть здесь по той же причине, что и `contextFreeze` выше: поток и главный поток
   * получают флаги ОДНИМ объектом, и поле, отсутствующее только тут, дало бы разное решение о
   * допуске на двух прод-дорогах. Это расхождение не в числах, а в том, ЧТО исполнялось, —
   * и потому опаснее.
   */
