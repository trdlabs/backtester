...
/**
 * `multi_symbol_risk` — профиль для многосимвольных actor-прогонов (083 S3 ступень 1).
 *
 * ЗАЧЕМ ОТДЕЛЬНЫЙ, А НЕ ПРАВКА ДЕФОЛТА. Решение владельца 2026-08-17: `DEFAULT_RISK` не трогать.
 * Поднять лимит в нём значило бы сменить семантику КАЖДОГО существующего прогона — односимвольные,
 * написанные под «одна позиция», молча получили бы другое правило.
 *
 * ПОЧЕМУ ИМЕННО 10, А НЕ «БЕЗ ЛИМИТА». Профиль без `maxConcurrentPositions` невозможен: контракт
 * объявляет поле обязательным. Но лимит здесь и не помеха — он ЧЕСТНО СОБЛЮДАЕТСЯ, пока символов
 * в прогоне не больше десяти: односимвольный актор держит не больше одной позиции по построению
 * (`openPositionsOf`), значит портфель не превысит числа акторов. Прогон на одиннадцати символах
 * под этим профилем будет отвергнут — и это правильно, а не досадно: одиннадцатая позиция вышла бы
 * за объявленное, и удержать её было бы нечем.
 *
 * Число выбрано как рабочий потолок корзины, а не как «побольше»: оно ОБЯЗАНО быть конечным и
 * обязано отвергать то, чего не гарантирует. Нужна корзина шире — это новый профиль с новым
 * числом и новым решением о том, что мы готовы держать одновременно.
 *
 * Ограничение на РАЗМЕР позиции сохранено: `exposureLimits` применяется к каждому актору и в
 * координаторе не нуждается — это правило про одну позицию, а не про их множество.
 */
export const MULTI_SYMBOL_RISK: RiskProfile = {
