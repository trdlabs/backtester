...
function collectEnvIssues(env: NodeJS.ProcessEnv): string[] {
  ...
  // POC (analysis/18 A): валидный бэкенд sandbox + несовместимость isolate/universe.
  const backend = env.BACKTESTER_SANDBOX_BACKEND ?? 'docker';
  if (backend !== 'docker' && backend !== 'isolate') {
    issues.push(`BACKTESTER_SANDBOX_BACKEND must be 'docker' or 'isolate', got '${backend}'`);
  }
  if (backend === 'isolate' && env.BACKTESTER_UNIVERSE_SESSION === 'true') {
    issues.push('BACKTESTER_SANDBOX_BACKEND=isolate does not support BACKTESTER_UNIVERSE_SESSION');
  }
