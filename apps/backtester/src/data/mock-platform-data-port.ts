...
interface CoverageEntry {
  ...
  /**
   * Однороден ли источник свечей внутри суток. ВЫЧИСЛЯЕТСЯ сервером из файлов дня, а не
   * заявляется писателем: рекордер сбрасывает много part-файлов за сутки, и передеплой между ними
   * меняет источник, о чём ни один отдельный файл не знает.
   */
  ...
  async listDatasets(): Promise<DatasetDescriptor[]> {
    ...
    return snapshot.entries
      ...
      .map((e) => ({
        ...
        rowCount:   e.barCount,
        // ИМЯ ВЕНЬЮ ПРИНИМАЕТСЯ ТОЛЬКО В ПАРЕ С ФЛАГОМ ОДНОРОДНОСТИ, и это не педантизм.
        //
        // Прувер ниже трактует отсутствие флага как «вопрос не задавался» — иначе фикстура,
        // объявившая венью, стала бы неоднородной от одного добавления параметра. У СЕРВЕРА такого
        // послабления быть не может: сервер, назвавший венью и промолчавший про однородность,
        // сообщил ПОЛОВИНУ факта, а половина здесь неотличима от целого — имя выглядит одинаково и
        // при однородных сутках, и при собранных из двух источников.
        //
        // Такая пара возможна только при рассинхроне версий (старый сервер, новый клиент), и
        // именно поэтому проверяется ЗДЕСЬ: этот порт разговаривает с сервером и умеет отличить
        // его от фикстуры, а прувер — нет.
        //
        // `candleVenue` при неполном объявлении НЕ пробрасывается вовсе: ниже по цепи это станет
        // «происхождение неизвестно» и прогон будет отвергнут — то есть безопасное поведение
        // получается само, без нового класса отказа.
        ...(e.candleVenue !== undefined && e.candleVenueHomogeneous !== undefined
          ? { candleVenue: e.candleVenue }
          : {}),
      }));
  }
