...
- [shared-execution-engine](../../control-center/docs/delivery/initiatives/shared-execution-engine.md) — `implementing`. One deterministic execution core for backtester + platform, shipped as a new public `@trdlabs/engine` (revises Q1-083 "engine-core inside the SDK"; `@trdlabs/backtester-sdk` stays standalone — doc 07 A+ upheld). Backtester part:
  ...
  - ⏭️ **Next (S2, in `trdlabs/engine`, not here):** scheduler + `seq`, order FSM, execution ledger, advance-time timers, sim-exchange, checkpoint format, engine-level recovery-equivalence. **Blocking constraint for that work:** `@trdlabs/engine@0.3.0` still pins `@trdlabs/sdk@0.13.0`, so this repo currently resolves two copies of the SDK. Harmless today (the v1 engine never touches the branded µs types), fatal once the engine adopts the actor contract — two installs mean two nominal brand identities. S2's first commit must be the engine's own `0.14.0` pin.
