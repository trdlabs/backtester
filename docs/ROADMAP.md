...
- [wfo-extended-fixture](../../control-center/docs/delivery/initiatives/wfo-extended-fixture.md) — `proposed`. Backtester part: tier-aware fixture selection for WFO/novelty/holdout runs — declare the required history depth up front and fail fast naming the required tier. Today the 6.94-day default fixture makes the novelty gate unsatisfiable (`noveltyMinOverlapDays` 30, `config.ts:489-492`) and WFO folds degenerate (test fold ≈ 1.1 days at F=5); failures surface deep in the contour instead. Consumer facts feeding the 42-day tier design: worst committed warmup default MACD 34 bars (`engine/indicators/catalog.ts`), `holdoutFraction` 0.2, `walkForwardMaxFolds` 20.
  - ✅ **DONE (backtester part, branch `feat/wfo-tier-failfast`)** — committed `snapshot-tiers.json` (control-center `pnpm sync` output) + a fail-closed reader (`data/snapshot-tier-catalog.ts`, throws on missing file / unrecognized `schemaVersion`) + `requiredTierForDays`/`formatTierHint`. All three advisory resolvers (`resolveWalkForward`/`resolveNovelty`/`resolveHoldoutMarker`, `jobs/worker.ts`) now run an up-front (pre-deep-work) history-sufficiency check (`engine/required-history.ts`) against the requested period's own span and, on a miss, return their EXISTING advisory status with a new additive `reason: 'insufficient_history'` + `requiredDays`/`requiredTier` — never a rejection; short-window research runs are unaffected. `resultHash` unaffected (fields ride the post-hash summary projection, same as every other advisory field).
...
- [shared-execution-engine](../../control-center/docs/delivery/initiatives/shared-execution-engine.md) — `implementing`. One deterministic execution core for backtester + platform, shipped as a new public `@trdlabs/engine` (revises Q1-083 "engine-core inside the SDK"; `@trdlabs/backtester-sdk` stays standalone — doc 07 A+ upheld). Backtester part:
  - ✅ **Phase 2 — donor (done 2026-07-24).** The engine core was extracted from this repo's engine layer: `engine/{execution,portfolio,risk,market-tape,profiles}.ts`, `determinism/*` (seeded rng, canonical-json/quantize), the R8 intra-bar order (`preBarStages`/`processBar`) — with zero commits to the donors. The research harness (walk-forward, holdout, promotion gate, jobs/worker, sandbox) stayed local; the seam was already `RunDeps`/`finalizeResult`.
  - ✅ **Phase 3 — first consumer (done 2026-07-26, #163).** `apps/backtester` runs on `@trdlabs/engine` (version pin, not submodule). Extraction-equivalence proven as a TEST, not a claim: rolling back ONLY the Ф3-added shape keys reproduces every pre-Ф3 golden exactly (`test/engine-extraction-migration.test.ts` + `test/fixtures/f3-engine-migration/hash-map.json`); twin-equivalence trusted ≡ sandbox green on live Docker. `packages/research-contracts` still exists as the parity anchor — its retirement is not done.
  - ✅ **Ф6/S1 consumed (done 2026-08-10, #225).** `@trdlabs/sdk@0.14.0`, contract `017.3 → 017.4`. Not a dependency bump: `contractVersion` rides in `RunEvidence`, which is part of the canonical payload, so the version move rebases **every** committed `result_hash`. Goldens were rebased under proof — the migration chain is now `017.2 → 017.3 → Ф3 → 017.4`, each link asserting the previous one, and only its **head** may write the shared golden files (`test/helpers/migration-chain.ts`).
  - ✅ **S2 landed upstream and is consumed here (2026-08-11, #227 → #228 → #229).** The actor core lives in `trdlabs/engine`; this repo took it in three exact-pin steps (`0.4.0` → `0.6.0` → joint `sdk@0.15.0` + `engine@0.7.0`). The old blocking constraint is **discharged**: the engine now pins the contract exactly, and the "one nominal identity for the branded µs types" property is held by a **gate**, not by observation — `test/engine-pin-single-sdk.test.ts` compares the physical directories that resolution lands on from the app and from the installed engine. It has to be a gate because this repo's own manifests use `^0.14.0`: the day `0.14.1` ships, two copies appear with no edit here. Every step of that chain was verified from npm independently (version present, contract pin exact, actor surface callable from the tarball, one copy in a clean consumer tree, provenance attached).
  - ✅ **S3 preparatory slice — fail-closed (done 2026-08-12, #230).** The lifecycle seam exists (`createActor → executeActorEvent → disposeActor` over an opaque `ActorExecutionHandle`) and **nothing executes through it yet**. Admission is split in two so that a refusal cannot leak a router: the pure part (`manifest.lifecycle`, rollout flag, `BAR_BATCHING`, `barMajorBatch`) runs *before* the executor is built; the capability check and the standing projection refusal run inside the `try` that owns `closeAll()`. No condition set falls through to legacy — with the flag on, compatible modes and a capable executor, the run is still rejected with `unsupported_lifecycle` and an empty JSON Pointer, before init, because the ledger → artifacts projection is not connected. `runEventDrivenSymbol` is deliberately unwired and legacy `runSymbol` is untouched.
  - ✅ **Flag delivery is proven, not inferred.** `BACKTESTER_EVENT_DRIVEN_ENABLED` is a declared production capability, so its transport is tested end to end: `AppConfig → WorkerDeps` by assertion (not typecheck), and both queue roads — direct and thread. One shared `strategyRunFlags()` builds the run flags for the queue path and for walk-forward, after those two hand-kept mirrors had already diverged (walk-forward silently dropped the flag, so the same strategy would be admitted by a plain run and refused by a folded one).
  - ✅ **Ledger → artifacts projection (done 2026-08-12, #232).** A pure, *checking* layer: it does not merely re-shape fields, it re-folds the recorded journal with the engine's own `applyFill`/`applyFunding` and requires the result to equal the recorded `finalLedger` in full — including the fills array field by field. Trades are **derived by the engine** (`@trdlabs/engine@0.9.0`, `deriveActorTrades`) from one ordered accounting journal (`fill | funding`); this repo supplies only what nothing else knows — the close *reason*, keyed by the closing fill's id. Two deliberately different accountings then have to agree (`reconcileRealizedPnl`): the ledger realizes entry fees immediately, a trade defers them to the close, so an error in either half is visible only on their equality.
  - Sufficiency of the ledger alone was **checked and disproven** for all seven artifact families: `baseOpen`/`slippageBps` exist only at match time, `closeReason` and the RiskEngine verdicts are host policy, equity is per-bar, and `applyFunding` folds `cost` into `realizedPnl` keeping no record at all. Hence an explicit input aggregate — which is not a container for the projector's convenience but the **list of what the hot runner must record while the fact still exists**.
  - ✅ **Actor timeline (done 2026-08-12, #235 + #236).** An append-only dispatch stream with a real gap/duplicate continuity guard on `seq` — this is the first place where §3.5's continuity is checkable at all: the frontier axis only ever sees endpoints, and a gap *inside* a frontier does not move them. It is **not** a second canonical trace: a rejected command leaves no trace in the artifacts whatsoever, so a timeline reconstructed from results would lie exactly where it is needed. Causation points **backwards** (append-only cannot know future `seq`), the event and the command are stored as the contract's own types in full, delivery identity comes from `ActorEnvelope`, and a successful run without a timeline is impossible — the check lives *inside* the projection, not beside it, because a gate the caller can forget to call guarantees nothing.
  - ⏭️ **Next, in this order (owner's sequencing):** queue observability of the refusal → and only then the hot event-driven runner (symbol-major and unbatched bar-major; `barMajorBatch` stays fail-closed until a separate multi-host composition). When the runner is wired, a separate gate must prove the timeline reaches the real `RunOutcome`/evidence, not merely the intermediate `ActorRunArtifacts`.
  - ✅ **Queue observability (done 2026-08-12, #239).** A rejected run's reason now survives into the job row as a structured `ValidationIssue[]` — `code`, `message`, `path` — read back **from the store**, not from what the worker returned. `terminalCode` deliberately stays `validation_error`: it answers *what kind of failure*, which was enough for the old taxonomy (`missing_dataset` is its own whole reason), so the detail discriminates and no reader breaks. Three refusals that differ **only** in the detail are proven on both transports — rollout disabled (an operator's setting), incompatible `BAR_BATCHING` (a run's configuration), executor without the actor lifecycle (the build's state); different people fix them, by different actions. The empty JSON Pointer survives as a key, checked both through the store and through an explicit JSON round-trip — in memory the loss would be invisible, in Postgres visible only there. Legacy runs on both transports finish and carry no reason at all: a reason on a completed run would mean we write it always and it discriminates nothing.
  - ⚠️ **Historical note (the tail this closed).** Through the queue a rejected run used to carry only `terminalCode: 'validation_error'` — the row has no message field and `timeline` carries no reason, so the `unsupported_lifecycle` code and its text die inside the thrown `RunnerError`. An operator looking at a refused event-driven run sees a generic validation failure and nothing else. Owner requirement: **before rollout**, the reason must survive into the terminal result or a structured timeline/log record.
  - 🚫 **Candle provenance — rollout blocker (found 2026-08-12, S3 subscription admission).** A run cannot prove which venue its candles came from, and that is a property of the recording, not a gap in this repo. Candles are written by **one** exchange adapter chosen at recorder deploy time via `MARKET_SOURCE_EXCHANGE` (legacy default `bybit`, `platform/src/config/market_layer.ts`), and that choice never reaches the row: `CanonicalRow` carries `symbol`, OHLCV and coverage flags. OI, funding and liquidations are the opposite — **aggregated** across venues (`oi_total_usd` "Aggregated OI", `funding_rate` "Aggregated funding") — so a dataset has no single venue even in principle, and the question is meaningful only about candles. Hence the field is `candleVenue`, not `venue`. `DatasetDescriptor` (here and in mock-platform) has no provenance field, and `datasetRef` is a free-form string. The gate is already fail-closed — `proveCandleVenue` returns `proven: false` and admission refuses before `createActor` — so **until the channel is restored end-to-end (recorder → archive → `DatasetDescriptor`), no real dataset can take the actor path**. It does not block the internal runner, which runs on a fixture with a proven `candleVenue`. Cross-repo owners: trading-platform, trading-backtester (control-center card row: *Candle provenance restored*, cc#365).
...
**Wave 2 (2026-08-05, cc `docs/analysis/27` + `docs/analysis/28`).** Canonical: the cards' «Дополнение 2026-08-05» sections; backtester-relevant slices only:

- **Р2+ (refines the novelty gate):** two thresholds, not one — a soft advisory threshold during search (novelty score surfaced into generator feedback) and the hard BRAIN-style one at promotion. Confirmed independently by XAlpha (0.95 generation / 0.60 library) and Chain-of-Alpha (Diversity feedback axis).
- **Battery additions Б1–Б3** ([research-validation-hardening](../../control-center/docs/delivery/initiatives/research-validation-hardening.md)) need backtester run-support: Б1 adversarial lookahead scan (re-run on ledger slices + decision diff — the sweep confirmed freqtrade's `lookahead-analysis` is the only tool of its kind; our dead-stdin protection is structural and does not catch leakage inside bundle data/logic), Б2 luck-baseline runs (N random strategies, same trading profile), Б3 cost-stress runs (fees + funding ×1.5/×2).
- **SEM-5** ([engine-semantics-hardening](../../control-center/docs/delivery/initiatives/engine-semantics-hardening.md)): fill-model check against the FINSABER bar (liquidity cap ≈2.5 % of 20-day ADV, next-open vs same-bar semantics, slippage) — after the perf epic. Same review's anti-lesson: LEAN's defaults (fees 0 for crypto, slippage 0) are inherited by every generated run — the perf-bench-fidelity lesson generalized.

**Wave 3 (2026-08-10, cc `docs/analysis/31` — Vibe-Trading read from source).** Canonical: the cards' «Дополнение 2026-08-10» sections; backtester-relevant slices only:

- **Б1/Б2 implementation samples (ВТ2/ВТ3):** Vibe-Trading proves the Б1 pattern is cheap — poison-the-future invariance (snapshot decision at bar T, corrupt all inputs from T+k, past must be byte-invariant) runs in their CI on 100 % of a 462-factor catalog (`agent/tests/factors/test_lookahead.py`); our form stays «re-run on tape slices + decision diff». For Б2, their strict bench computes the random control on the same dates as the signal (inner-join over seeds) and assigns categories only against the control; carry over the API rail — the random-control parameter is keyword-only with **no default**, so forgetting it is a syntax error, not a silent skip.
- **SEM-6 (ВТ6):** a machine-readable assumptions block in every run's evidence — execution-model assumptions (fill model, funding mode, resolution limits, known simplifications) as an artifact field, not an SSOT-doc sentence; model: Vibe-Trading `perpetual_evidence.py` (`bar_extrema_order_unknown`, `single_asset_usdt`, …). Natural follow-up of the SEM-5 audit.
- **Landscape correction:** perp funding in a backtest is now modeled by three systems (freqtrade, NautilusTrader, Vibe-Trading strict mode — data-driven funding, mark prices, versioned maintenance brackets); the cc/28 conclusion stands — the isolation + mechanical determinism + funding intersection is still empty (they have funding, partial isolation, no determinism: no seed, no engine/version pinning in the run card). Their `quantlib` has DSR/CSCV/purged-CV implemented but wired into no decision path — the exact «statistics exist, verdict ignores them» gap our enforced battery closed.
