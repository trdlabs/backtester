...
топологией). Guard exempt'ит бандл ТОЛЬКО когда он реально маршрутизируется в sandbox — тем же predicate, что и
router (`provenance === 'bundle' && bundle !== undefined`): provenance — метаданные, не привилегия,
поэтому поддельный/неполный bundle-provenance БЕЗ handle не проскочит guard (иначе исполнился бы trusted
in-process и потёк). На engine-`ResolvedStrategy` добавлены `provenance?: 'trusted'|'bundle'` и opaque
`bundle?: unknown` (проставляются 019-registry; `createTrustedRegistry` ставит `provenance:'trusted'`).
