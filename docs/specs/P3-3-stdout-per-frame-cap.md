...
Учёт ведётся по байтам (`stderrBytes`), обрезка — `utf8ByteSlice` (никогда не режет multibyte-символ
посередине, откатываясь к границе code point), truncation отмечается флагом `stderrTruncated`
(маркер `…[truncated]` добавляется в `stderrText()`). Итог: для multibyte-UTF-8 stderr байтовая длина
tail ≤ `maxStderrBytes`, без `U+FFFD` на месте среза.
