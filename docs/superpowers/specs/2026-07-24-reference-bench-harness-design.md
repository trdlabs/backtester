...
## Поправка после ревью (2026-07-24, PR #160)

Спека выше описывает матрицу из четырёх вариантов и единый baseline `off`. Реализация от этого
отошла — по двум причинам, вскрытым ревью:

1. **Референс задаётся по флагу, а не «всё против `off`».** `BACKTESTER_BAR_MAJOR` меняет модель
   исполнения (per-symbol portfolio + equal-weight агрегация), поэтому его `result_hash` расходится
   с symbol-major дефолтом ПО ДИЗАЙНУ; сверять его с `off` — значит объявить дефект там, где его нет.
   Цепочка референсов: `bar_batching`→`off`, `bar_major`→сам себе, `bar_major_universe`→`bar_major`,
   `bar_major_batch`→`bar_major_universe`.
2. **Вариантов пять, а не четыре: добавлен `bar_major_universe`.** Транспортный коллапс Slice B
   работает только внутри universe-сессии (`executeStrategyHookBarMajor` без `universe.enabled`
   уходит в per-symbol lockstep). Без отдельного варианта «universe без batch» эффекты 17c и
   Slice B неразделимы — а они, как показал замер, дают 1.67× и 1.37× соответственно.

Дополнительно спека НЕ предусматривала (а следовало): проверку того, что флаг вообще задействовался.
Первая редакция станка передавала `barBatching: true` вместо `{ maxBars }` и гоняла Slice B без
universe — два флага молча исполняли baseline, а вердикт был зелёный. Теперь у каждого варианта есть
предикат `engagementProblem`, и прогон без следа флага в IPC-профиле объявляется недействительным.
