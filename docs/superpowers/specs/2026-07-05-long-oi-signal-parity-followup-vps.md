...
> **CORRECTION (verified after this doc was first drafted — the original "params differ / config sanitized" framing below was WRONG).**

1. **Params are IDENTICAL — the residual cause is a `ctx.market` (OI/liq) semantics gap, a platform live-adapter deviation, NOT config.** The `analysisByRun.<runId>.strategyConfig = {available:false, reason:'not_in_sanitized_export'}` field is a **HARDCODED schema placeholder** (`trading-mock-platform/tools/fetch-snapshot/fetch-snapshot.ts:800`) — NOT evidence of a real config. VERIFIED: the retired live bot created long_oi via `ModuleStrategyAdapter({cfg})` and NEVER passed `params` → `this.params = opts.params ?? DEFAULT_PARAMS` → it ran the module's own `DEFAULT_PARAMS`, byte-identical to the vendored copy (`platform/src/strategies/long_oi/params.ts` golden-era == pre-retirement == vendored, zero diff). So "params differ" was an unverified assumption — **wrong**. The real cause: the live adapter (`platform/src/runtime/module_host/module_strategy_adapter.ts::prepareBar`) fabricates OI/liq `= 0` on data gaps (`?? 0`, violating `platform/.../live-adapter.md:20` "gap → undefined") and windows OI by **call-sequence** not calendar minute; the backtester's `market-access.ts` is contract-correct. The close-reason flips + 6/8 divergence (11–24% entry-price deltas = different trigger bars, not a timing offset) come from THAT — the backtest is arguably *more* correct than the buggy live path. **2/8 trades reproduce cleanly, confirming the harness + engine are correct.** Full analysis + platform ask: `2026-07-05-long-oi-live-adapter-ctxmarket-handoff.md`.
2. **Trade-record granularity: leg vs position (real artifact; reconciling it is NOT sufficient).** The vendored module's `tp1Action:'partial_exit'` (`tp1ExitPercent:50`) emits TWO closed-trade records per position (TP1 partial + remainder), while the golden ledger records ONE per position. Aggregating the engine's legs (8 legs → 7 positions, confirmed) removes this artifact but does NOT close parity — the ctx.market divergence (item 1) remains. A decision-level comparison sidesteps both.
3. **Same comparison DAY (06-18); NOT a period/warmup problem.** The `06-12` `startedAtMs` was only when the paper process launched, not the data period; both sides compared 06-18. long_oi's warmup is only `candlesMin:30`. Config was never the issue (item 1).
